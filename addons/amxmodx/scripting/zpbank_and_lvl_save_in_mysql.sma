...
public SQL_LoadDebug()
{
	...
	get_cvar_string("rs_host", host, charsmax(host))
	get_cvar_string("rs_user", user, charsmax(user))
	get_cvar_string("rs_pass", pass, charsmax(pass))
	get_cvar_string("rs_db", db, charsmax(db))
	
    g_hDBTuple = SQL_MakeDbTuple( host, user, pass, db );
    ...
    if( !SQL_TableExists( g_hConnect, SQL_TABLENAME ) )
    {
        ...
        formatex( szQuery, charsmax( szQuery ), "SET NAMES UTF8; CREATE TABLE IF NOT EXISTS `%s` ( id INT(11) NOT NULL AUTO_INCREMENT, `Name` varchar( 32 )  NOT NULL, `Money` INT NOT NULL, `LVL` INT NOT NULL, `XP` INT NOT NULL, PRIMARY KEY(id))", SQL_TABLENAME );
...
public SQL_QueryConnection( iState, Handle: hQuery, szError[ ], iErrorCode, iParams[ ], iParamsSize )
{
    ...
    if( SQL_NumResults( hQuery ) < 1 )
    {
        ...
        formatex( g_szQuery, charsmax( g_szQuery ), "SET NAMES UTF8; INSERT INTO `%s` ( `Name`, `Money`, `LVL`, `XP`) VALUES ( '%s', '%d', '%d','%d');", SQL_TABLENAME, g_szName[ iPlayer ], zp_get_user_ammo_packs( iPlayer ), zp_get_user_level(iPlayer), zp_get_user_xp(iPlayer));
        SQL_ThreadQuery( g_hDBTuple, "SQL_ThreadQueryHandler", g_szQuery );
