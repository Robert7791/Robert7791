...
public ShowTimer(id)
{
	...
	{
		ShowSyncHudMsg(id, g_sync_hud, "%L", id, "KICK_HUD")
		return PLUGIN_CONTINUE
	}
	else if(!is_registered[id] && get_pcvar_float(g_regtime))
	{
		...
		{
			ShowSyncHudMsg(id, g_sync_hud, "%L", id, "REGISTER_AFTER")
			set_task(1.0, "ShowTimer", id+TASK_TIMER)
			return PLUGIN_HANDLED
		}

		ShowSyncHudMsg(id, g_sync_hud, "%L", id, g_player_time[id] > 1 ? "REGISTER_HUD" : "REGISTER_HUD_SEC", g_player_time[id])
	}
	else if(is_registered[id] && !is_logged[id])
	{
		if(g_player_time[id] == -1)
		{
			ShowSyncHudMsg(id, g_sync_hud, "%L", id, "LOGIN_AFTER")
			set_task(1.0, "ShowTimer", id+TASK_TIMER)
			return PLUGIN_HANDLED
		}

		ShowSyncHudMsg(id, g_sync_hud, "%L ", id, g_player_time[id] > 1 ? "LOGIN_HUD" : "LOGIN_HUD_SEC", g_player_time[id])
	}
...
public Messages(id)
{
	...
	if(!is_registered[id])
	{
		if(get_pcvar_float(g_regtime) != 0)
		{
			client_printcolor(id, "%L", id, "REGISTER_CHAT", prefix, get_pcvar_num(g_regtime))
		}
		else
		{
			client_printcolor(id, "%L", id, "YOUCANREG_CHAT", prefix)
		}
	}
	else if(!is_logged[id])
	{
		client_printcolor(id, "%L", id, "LOGIN_CHAT", prefix, get_pcvar_num(g_logtime))
	}
	else if(is_autolog[id])
	{
		if(get_pcvar_num(g_announce))
		{
			...
			client_printcolor(id, "%L", id, "LOG_LOGING_G", prefix, temp_name)
		}
		else
		{
			client_printcolor(id, "%L", id, "AUTO_LOGIN_CHAT", prefix)
		}
	}
}
/*==============================================================================
...
public AutoLoginPassword(id)
{
	...
	if(!equal(hash, password[id]))
	{
		...
		client_printcolor(id, "%L", id, "LOG_PASS_INVALID", prefix, attempts[id], get_pcvar_num(g_attempts))
...
AutoLogin(id, bool:enabled = true)
{
	...
	{
		is_autolog[id] = true
		client_printcolor(id, "%L", id, "AUTO_LOGIN_ON", prefix)

		if(get_pcvar_num(g_aulog_log))
		{
			log_to_file(log_file, "%L", id, "LOGFILE_AUTO_ON", g_client_data[id])
		}
	}
	else
	{
		is_autolog[id] = false
		client_printcolor(id, "%L", id, "AUTO_LOGIN_OFF", prefix)
		get_pcvar_string(g_setinfo_pr, pass_prefix, charsmax(pass_prefix))
		client_cmd(id, "setinfo %s ^"^"", pass_prefix)
		
		if(get_pcvar_num(g_aulog_log))
		{
			log_to_file(log_file, "%L", id, "LOGFILE_AUTO_OFF", g_client_data[id])
		}
	}
...
public MainMenu(id)
{
	...
	if(is_registered[id])
	{
		if(is_logged[id])
		{
			length += formatex(menu[length], charsmax(menu) - length, "%L", id, "MAIN_MENU_LOG", id, is_autolog[id] ? "AUTO_LOG_DIS" : "AUTO_LOG_EN")
			keys = MENU_KEY_1|MENU_KEY_4|MENU_KEY_5|MENU_KEY_9|MENU_KEY_0
		}
		else
		{
			length += formatex(menu[length], charsmax(menu) - length, "%L", id, "MAIN_MENU_REG")
	...
	{		
		if(get_pcvar_float(g_regtime) == 0)
		{
			length += formatex(menu[length], charsmax(menu) - length, "%L", id, "MAIN_MENU_NOTREG")
			keys = MENU_KEY_2|MENU_KEY_9|MENU_KEY_0
		}
		else
		{
			length += formatex(menu[length], charsmax(menu) - length, "%L", id, "MAIN_MENU_NOTREG_FORCE")
	...
	switch(key)
	{
		case 0:
		{
			...
			else
			{
				...
				client_printcolor(id, "%L", id, "LOG_OUT", prefix)
				is_logged[id] = false
			}
		}
		case 1:
		...
		case 3:
		{
			...
			{
				client_printcolor(id, "%L", id, "CHANGE_TIMES", prefix, get_pcvar_num(g_chp_time))
				MainMenu(id)
				return PLUGIN_HANDLED
			}
			else
...
public Login(id)
{
	...
	if(changing_name[id])
	{
		client_printcolor(id, "%L", id, "LOGIN_AFTER")
		return PLUGIN_HANDLED
	}

	if(!is_registered[id])
	{	
		client_printcolor(id, "%L", id, "LOG_NOTREG", prefix)
		return PLUGIN_HANDLED
	}

	if(is_logged[id])
	{
		client_printcolor(id, "%L", id, "LOG_LOGGED", prefix);
		return PLUGIN_HANDLED
	}
	...
	else
	{
		...
		{
			client_printcolor(id, "%L", id, "LOG_LOGING", prefix)
		}
...
public Register(id)
{
	...
	if(changing_name[id])
	{
		client_printcolor(id, "%L", id, "REGISTER_AFTER")
		return PLUGIN_HANDLED
	}
	...
	if(is_registered[id])
	{
		client_printcolor(id, "%L", id, "REG_EXISTS", prefix)
		return PLUGIN_HANDLED
	}

	if(passlength < get_pcvar_num(g_pass_length))
	{
		client_printcolor(id, "%L", id, "REG_LEN", prefix, get_pcvar_num(g_pass_length))
		client_cmd(id, "messagemode REGISTER_PASS")
		return PLUGIN_HANDLED
	}
...
public ConfirmPassword(id)
{
	...
	formatex(menu, charsmax(menu) - length, "%L", id, "MENU_PASS", new_pass[id])
	...
	switch(key)
	{
		case 0:
		{
			...
			if(is_registered[id])
			{
				...
				client_printcolor(id, "%L", id, "CHANGE_NEW", prefix, new_pass[id])
				...
				{
					log_to_file(log_file, "%L", id, "LOGFILE_CHNG_PASS", g_client_data[id])
				}
			...
			else
			{
				...
				if(get_pcvar_num(g_announce))
				{
					client_printcolor(id, "%L", id, "REG_REGIS_G", prefix, temp_name)
				}
				else
				{
					client_printcolor(id, "%L", id, "REG_REGIS", prefix, get_pcvar_num(g_logtime))
				}
				
				if(get_pcvar_num(g_reg_log))
				{
					log_to_file(log_file, "%L", id, "LOGFILE_REG", g_client_data[id])
				}
...
public client_command(id)
{
	...
	if((equali(command, "say") || equali(command, "say_team")) && equali(arg, "/reg"))
	{
		if(!name_checked[id])
		{
			client_printcolor(id, "%L", id, "WHITE_LIST", prefix)
		}
		else
		{
			MainMenu(id)
	...
	if(get_pcvar_num(g_comm) == 1)
	{
		if(TrieKeyExists(g_commands, command))
		{
			...
			{
				console_print(id, "%s %L", prefix, id, "COMMAND_REG")
				client_printcolor(id, "!g%s!t %L", prefix, id, "COMMAND_REG")
				return PLUGIN_HANDLED
			}
			else if(is_registered[id] && !is_logged[id])
			{
				console_print(id, "%s %L", prefix, id, "COMMAND_LOG")
				client_printcolor(id, "!g%s!t %L", prefix, id, "COMMAND_LOG")
				return PLUGIN_HANDLED
			}	
		}
	}
	...
	if(!equal(oldname, newname))
	{
		...
		else
		{
			if(is_logged[id])
			{
				...
				client_printcolor(id, "%L", id, "NAME_CHANGE_LOG", prefix)
...
public KickPlayer(parameters[])
{
	...
	switch(reason)
	{
		case 1:
		{
			...
			console_print(id, "%L", id, "KICK_INFO")
		...
		{
			console_print(id, "%L", id, "KICK_INFO")
			server_cmd("kick #%i ^"%L^"", userid, id, "KICK_LOGOUT")
		}
	}
	return PLUGIN_CONTINUE
}
...
public RemoveCantChangePass(data[])
{
	...
	client_printcolor(target, "%L", target, "CHANGE_CAN", prefix)
	return PLUGIN_CONTINUE
}
/*==============================================================================
